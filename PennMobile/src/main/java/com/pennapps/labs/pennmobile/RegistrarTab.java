...
public class RegistrarTab extends SearchFavoriteTab {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
            frameID = Companion.generateViewId();
        } else {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (getFav()) {
            ...
            getSearch_instructions().setText(getString(R.string.registrar_instructions));
        }
    }

    @Override
    public void onListItemClick(ListView l, View v, int position, long id) {
        if (!(getMListView().getAdapter() instanceof RegistrarAdapter)) {
            String s = getMListView().getAdapter().getItem(position).toString();
        ...
        args.putBoolean(getString(R.string.registrar_search), getFav());
    ...
    private void setBackButton(FrameLayout frameLayout) {
        ...
        frameLayout.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN && fragments[getFav() ? 1 : 0] != null) {
                    backRemove(getFav() ? 1 : 0);
                    return true;
                }
                if (((getFav() && fragments[0] != null && fragments[1] == null) || (!getFav() && fragments[0] == null && fragments[1] != null)) &&
                        (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN)) {
                    backRemove(getFav() ? 0 : 1);
                }
                return false;
            }
        });
                ...
                .subscribe(new Action1<List<Course>>() {
                    ...
                    public void call(List<Course> courses) {
                        ...
                        if (courses.isEmpty()) {
                            ...
                            mAdapter = new RegistrarAdapter(getMActivity(), filterCourses(courses));
