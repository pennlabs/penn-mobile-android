...
class SubletDetailsPagerAdapter(fragment: Fragment, private val dataModel: SublettingViewModel) : FragmentStateAdapter(fragment) {

    override fun getItemCount(): Int = 2
    override fun createFragment(position: Int): Fragment {
        return if (position == 0) {
            SubletDetailsFragment(dataModel)
        } else {
            SubletCandidatesFragment(dataModel)
        }
    }
